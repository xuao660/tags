...
//        });
//
//        List<User> users = userMapper.selectList(queryWrapper).stream().map(this::getSafetyUser).collect(Collectors.toList());
        ...
        Gson gson = new Gson();
        // filter对stream流中的元素进行过滤，如果filter（）中返回true就把这个元素留下
        List<User> users = userList.stream().filter(user -> {
            String tags = user.getTags();
            //如果用这段代码判空，就不需要下面的optional判断
            ...
            return tagNameList.stream().allMatch(tagSet::contains);
